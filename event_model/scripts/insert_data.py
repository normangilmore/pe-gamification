...
engine = create_engine(
    "postgresql://pe_dashboard:test-only-yVu8W5azUtZ8RPSWX42o@localhost:5432/pe_dashboard", echo=True)
...
def insert_data(filename):
    with open(filename, newline='') as f:
        ...
        for row in reader:
            ...
            in_user = session.query(Profile).filter(
                Profile.id == row['user_id']).first()
            if in_user is None:
                u = Profile(id=row['user_id'], name=row['name'],
                            email_addr=row['email_addr'])
                session.add(u)
                session.commit()
            # If project not in project table, then add
            in_proj = session.query(Project).filter(
                Project.id == row['project_id']).first()
            if in_proj is None:
            ...
            in_task = session.query(Task).filter(
                Task.id == row['task_id']).first()
            if in_task is None:
