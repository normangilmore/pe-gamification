...
def fill_taskrun(project_ids, write=True):
    ...
    for project_id in project_ids:
        ...
        for task_run in task_runs:
            task_dict[task_run['id']] = [task_run['created'], task_run['project_id'], task_run['task_id'],
            task_run['user_id'], task_run['finish_time'], task_run['info']]
            index += 1
    if write:   
        with open('taskrun.csv', 'w') as f:
            writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(["id", "created", "project_id", "task_id", "user_id", "finish_time", 'task'])
            for i in task_dict:
                tr = task_dict[i]
                if type(tr[5]) is dict:
                    writer.writerow([i, tr[0], tr[1], tr[2], tr[3], 
                                     tr[4], tr[5]['highlight_group']['topic_name']])
    else:
        return task_dict
