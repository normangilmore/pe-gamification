...
def get_ids(project_ids):
	...
	they've completed for the given proejcts
	"""
	for project_id in project_ids:
		r = requests.get('https://pe.goodlylabs.org/api/taskrun?api_key=' + api_key + 
			'&project_id=' + project_id + '&orderby=id', headers=headers)
...
def get_names(user_ids):
	...
	for user in user_ids:
		r = requests.get('https://pe.goodlylabs.org/api/user/' + user + '?api_key=' + api_key, headers=headers)
